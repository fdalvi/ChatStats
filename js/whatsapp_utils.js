...
// This regex matches non-printable characters like E280AA (Left-to-Right Encoding) that WhatsApp includes
...
function checkFileFormat(file) {
	...
	reader.onload = function(e) {
		...
		var current_line = "";
		var previous_line = "";
		var state = STATES.META
		for (var c_idx in text) {
			var c = text[c_idx];
			character_count += 1;
			if (c != '\n') {
				current_line += c
			} else {
				// console.log(current_line.length);
				var match = current_line.match(/^(\d\d?\/\d\d?\/\d\d?,\s\d\d?:\d\d?:\d\d?\s.M):\s([^:]+):(.+)/)
				if (match != null) {
					var username = match[2].replace(non_printable_re, "");
					var num_tokens = match[3].match(/\s+/g);
					num_tokens = num_tokens?num_tokens.length:1;
					if (username in users) {
						users[username] += 1;
						messages[username].push({date: new Date(match[1]), message: match[3], num_tokens: num_tokens})
					}
					else {
						users[username] = 1;
						
						messages[username] = [{date: new Date(match[1]), message: match[3], num_tokens: num_tokens}]
					}
				} else {
					// current_lineprevious_line
				}
		...
		// 	var first_name = user.split(' ')[0]
		// 	$("#user-elements").append("<div class='user'> \
		// 			<!-- <div class='profile'></div> --> \
		// 			<div class='username'> \
		// 				" + first_name +" \
		// 			</div> \
		// 			<div class='tags'> \
		// 				<input class='tags-element' value='" + first_name + "," + user + "' /> \
		// 			</div> \
		// 		</div>");
		// }
...
function computeStatistics(file) {
	...
	var num_messages = all_users.map(name => users[name]);
	sort_by_second(all_first_names, num_messages);

	processed_data['total_messages_overall'] = {labels: all_first_names, data: num_messages};

	// ranking by messages for last year
	var all_first_names = all_users.map(name => name.split(' ')[0]);
	var reduce_by_last_year = (acc, data) => (data.date - last_year_date > 0) ? acc + 1 : acc;
	var num_messages_last_year = all_users.map(name => messages[name].reduce(reduce_by_last_year, 0));
	sort_by_second(all_first_names, num_messages_last_year);

	processed_data['total_messages_lastyear'] = {labels: all_first_names, data: num_messages_last_year};

	// ranking by words
	var all_first_names = all_users.map(name => name.split(' ')[0]);
	var reduce_accumulate = (acc, data) => acc + data.num_tokens;
	var num_words = all_users.map(name => messages[name].reduce(reduce_accumulate, 0));
	sort_by_second(all_first_names, num_words);

	processed_data['total_words_overall'] = {labels: all_first_names, data: num_words};

	var all_first_names = all_users.map(name => name.split(' ')[0]);
	var reduce_by_last_year = (acc, data) => (data.date - last_year_date > 0) ? acc + data.num_tokens : acc;
	var num_words_last_year = all_users.map(name => messages[name].reduce(reduce_by_last_year, 0));
	sort_by_second(all_first_names, num_words_last_year);

	processed_data['total_words_lastyear'] = {labels: all_first_names, data: num_words_last_year};

	// message length
	var all_first_names = all_users.map(name => name.split(' ')[0]);
	var reduce_accumulate = (acc, data) => acc + data.message.length;
	var message_length = all_users.map(name => messages[name].reduce(reduce_accumulate, 0));
	sort_by_second(all_first_names, message_length);

	processed_data['message_length_overall'] = {labels: all_first_names, data: message_length};

	var all_first_names = all_users.map(name => name.split(' ')[0]);
	var reduce_by_last_year = (acc, data) => (data.date - last_year_date > 0) ? acc + data.message.length : acc;
	var message_length_last_year = all_users.map(name => messages[name].reduce(reduce_by_last_year, 0));
	sort_by_second(all_first_names, message_length_last_year);

	processed_data['message_length_lastyear'] = {labels: all_first_names, data: message_length_last_year};
	
	// who packs the most punch (avg words per message)
	var all_first_names = all_users.map(name => name.split(' ')[0]);
	var message_density = all_first_names.map(name => processed_data['total_words_overall'].data[processed_data['total_words_overall'].labels.indexOf(name)]/processed_data['total_messages_overall'].data[processed_data['total_messages_overall'].labels.indexOf(name)]);
	sort_by_second(all_first_names, message_density);

	processed_data['message_density_overall'] = {labels: all_first_names, data: message_density.map(x => x.toFixed(2))};

	var all_first_names = all_users.map(name => name.split(' ')[0]);
	var message_density_last_year = all_first_names.map(name => processed_data['total_words_lastyear'].data[processed_data['total_words_lastyear'].labels.indexOf(name)]/processed_data['total_messages_lastyear'].data[processed_data['total_messages_lastyear'].labels.indexOf(name)]);
...
function drawPlots(file) {
	...
	window.chartColors = {
		...
		light_green: color('rgb(75, 192, 192)').alpha(0.5).rgbString(),
		blue: 'rgb(54, 162, 235)',
		light_blue: color('rgb(54, 162, 235)').alpha(0.5).rgbString(),
		purple: 'rgb(153, 102, 255)',
		grey: 'rgb(201, 203, 207)'
	};

	$("#all_charts").removeClass("hidden")

	console.log("Drawing All Messages")
	$("#total_messages").removeClass("hidden");
	plot_dual_bar_graphs("total_messages_overall", "Total Messages", "total_messages_lastyear", "Total Messages in the last year")

	console.log("Drawing Words")
	$("#total_words").removeClass("hidden");
	plot_dual_bar_graphs("total_words_overall", "Total Words", "total_words_lastyear", "Total Words in the last year")

	console.log("Drawing Message Length")
	$("#message_length").removeClass("hidden");
	plot_dual_bar_graphs("message_length_overall", "Message Length in characters", "message_length_lastyear", "Message Length in the last year")
...
function sort_by_second(first, second) {
	first.map(function (v, i) {
		return {
			value1: v,
			value2: second[i]
		};
	}).sort(function (a, b) {
		return ((a.value2 < b.value2) ? 1 : ((a.value2 == b.value2) ? 0 : -1));
